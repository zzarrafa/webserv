...
std::vector<std::string> Cgi::split_cgi(const std::string &str, const std::string &delimiter)
{
...
std::pair<std::string, std::map<std::string, std::string> > Cgi::outtputt(std::string output)
{
	...
	while (ss.good())
	{
		...
		if (line.length() > 0)
		{
			...
			if (tab[0] == "Set-Cookie")
			{
				...
				std::cout << "zabobi " << headers[tab[0]] << std::endl;
...
std::pair<std::string, std::map<std::string, std::string> > Cgi::executi_cgi(request req, char **av, std::string output)
{
	int fd[2];
	int nfd[2];
	char c;
...
char **Cgi::arguments(std::string cgi, std::string path)
{
